...
async function getOrderByOrderIdWithClient(client, orderId) {
  const orderResult = await client.query(
    `
      select *
      from orders
...
export async function getOrderByOrderId(orderId) {
  ...
  return withClient((client) =>
    getOrderByOrderIdWithClient(client, normalizedOrderId),
  );
}
...
export async function confirmOrder(orderId, payload = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    return getOrderByOrderIdWithClient(client, normalizedOrderId);
  });
}
