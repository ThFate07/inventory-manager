...
function validateOrderInput(payload) {
  ...
  for (const item of payload.items) {
    ...
    if (item.unitPriceInr != null && item.unitPriceInr !== "") {
...
function normalizeRequestedOrderItems(items) {
  ...
  for (const item of Array.isArray(items) ? items : []) {
    ...
    const unitPriceInr =
      item?.unitPriceInr == null || item?.unitPriceInr === ""
        ? null
        : Number(item.unitPriceInr);
    ...
    mergedItems.set(productId, {
      quantity: (mergedItems.get(productId)?.quantity || 0) + quantity,
      unitPriceInr: Number.isFinite(unitPriceInr) && unitPriceInr >= 0 ? unitPriceInr : null,
    });
  }

  return [...mergedItems.entries()].map(([productId, value]) => ({
    productId,
    quantity: value.quantity,
    unitPriceInr: value.unitPriceInr,
  }));
}
...
function normalizeEditableOrderItems(items) {
  ...
  for (const item of items) {
    ...
    if (unitPriceInr != null && (!Number.isFinite(unitPriceInr) || unitPriceInr < 0)) {
      throw new Error("Unit price must be a non-negative number.");
    }
    ...
    mergedItems.set(productId, {
      ...
      unitPriceInr: unitPriceInr ?? previousItem.unitPriceInr,
    });
  }
...
async function getProductsForOrderMutation(client, productIds) {
...
async function listOrdersInternal(limit) {
...
export async function listRecentOrders(limit = 12) {
  return listOrdersInternal(limit);
}
...
export async function createPendingOrder(payload) {
  ...
  return withTransaction(async (client) => {
    ...
    const normalizedItems = requestedItems.map((item) => {
      ...
      const requestedUnitPriceInr = Number(item.unitPriceInr);
      const unitPriceInr =
        Number.isFinite(requestedUnitPriceInr) && requestedUnitPriceInr >= 0
          ? requestedUnitPriceInr
          : Number(product.unit_price_inr);
      ...
      return {
        ...
        unitPriceInr,
        lineTotalInr: Number(item.quantity) * unitPriceInr,
      };
    });
...
export async function updateOrder(orderId, payload = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    const requestedItems = normalizeEditableOrderItems(payload.items);
    const existingItems = await getOrderItemsForMutation(client, order.id);
    const existingItemsByProductId = new Map(
      existingItems.map((item) => [Number(item.productId), item]),
    );
    const requestedProductsById = await getProductsForOrderMutation(
      client,
      requestedItems.map((item) => Number(item.productId)),
    );

    for (const item of requestedItems) {
      if (!requestedProductsById.has(Number(item.productId))) {
        throw new Error("A selected product no longer exists.");
      }
    }

    const normalizedItems = requestedItems.map((item) => {
      const product = requestedProductsById.get(Number(item.productId));
      const previousItem = existingItemsByProductId.get(Number(item.productId));
      const providedUnitPrice =
        item.unitPriceInr == null ? null : Number(item.unitPriceInr);

      let unitPriceInr;

      if (order.status === "confirmed") {
        const expectedUnitPriceInr = previousItem
          ? Number(previousItem.unitPriceInr)
          : Number(product.unit_price_inr);

        if (
          providedUnitPrice != null &&
          Number(providedUnitPrice) !== Number(expectedUnitPriceInr)
        ) {
          throw new Error("Reverse the confirmed order before changing item prices.");
        }

        unitPriceInr = expectedUnitPriceInr;
      } else {
        unitPriceInr =
          providedUnitPrice != null && Number.isFinite(providedUnitPrice)
            ? providedUnitPrice
            : previousItem
              ? Number(previousItem.unitPriceInr)
              : Number(product.unit_price_inr);
    ...
    if (order.status === "confirmed") {
      for (const item of normalizedItems) {
        ...
        const quantityDelta = Number(item.quantity) - previousQuantity;

        if (quantityDelta > 0 && item.stockQuantity < quantityDelta) {
          throw new Error(`Not enough stock for ${item.productName}.`);
        }
      }
      ...
      for (const item of normalizedItems) {
        ...
        await client.query(
          `
            ...
            where id = $1
          `,
          [item.productId, quantityDelta],
        );

        await insertInventoryLog(client, {
          action: quantityDelta > 0 ? "stock_deducted_from_order" : "stock_restored_from_order",
          productId: item.productId,
          productCode: item.productCode,
          productName: item.productName,
          category: item.category,
          details:
            quantityDelta > 0
              ? `Deducted ${quantityDelta} additional units while updating order ${normalizedOrderId}.`
              : `Restored ${Math.abs(quantityDelta)} units while updating order ${normalizedOrderId}.`,
        });
      }
    }
    ...
    const totalAmountInr = calculateOrderTotal(normalizedItems);
    ...
    await client.query(
      `
        ...
        where id = $1
      `,
      [order.id, totalAmountInr],
    );

    await insertInventoryLog(client, {
      action: "order_updated",
      details: `Updated order ${normalizedOrderId} for ${order.customer_name} to ${normalizedItems.length} item${normalizedItems.length === 1 ? "" : "s"} totaling ${formatCurrencyForLog(totalAmountInr)}.`,
    });
