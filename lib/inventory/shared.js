...
const inventoryLogColumnSupportPromises = new Map();

export async function inventoryActivityLogSupportsColumn(columnName) {
  if (!inventoryLogColumnSupportPromises.has(columnName)) {
    inventoryLogColumnSupportPromises.set(
      columnName,
        ...
        const result = await client.query(
          `
            ...
            where table_schema = 'public'
              and table_name = 'inventory_activity_logs'
              and column_name = $1
            limit 1
          `,
          [columnName],
  ...
  return inventoryLogColumnSupportPromises.get(columnName);
}

export async function inventoryLogSupportsImportBatchId() {
  return inventoryActivityLogSupportsColumn("import_batch_id");
}

export async function inventoryLogSupportsPayload() {
  return inventoryActivityLogSupportsColumn("payload");
}
...
export async function insertInventoryLog(client, entry) {
  ...
  if (supportsImportBatchId && supportsPayload) {
    await client.query(
      `
